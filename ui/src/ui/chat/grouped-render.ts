...
export function renderMessageGroup(
  ...
  const firstMsgText = firstMessage ? (() => {
    const m = firstMessage as Record<string, unknown>;
    const c = m.content;
    if (Array.isArray(c)) {
      return c.filter((b: unknown) => typeof (b as Record<string, unknown>).type === "string" && (b as Record<string, unknown>).type === "text")
  ...
  const isCompactionNoticeGroup =
    ...
    resolveCompactionThreadNoticeText(firstMessage, firstMsgText) != null;
  const who =
    normalizedRole === "user"
...
function renderGroupedMessage(
  ...
  const compactionNotice = resolveCompactionThreadNoticeText(message, extractedText || null);
  if (compactionNotice) {
    return html`
