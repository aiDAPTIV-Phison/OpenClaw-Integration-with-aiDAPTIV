...
export async function loadChatHistory(state: ChatState) {
  ...
  try {
    ...
    let merged = preserveOptimisticTailMessages(visibleMessages, previousMessages);

    // Preserve pending optimistic user merge (text-only sends stale during compaction/hybrid retries)
    const pendingUser = (state as ChatState).pendingOptimisticUserMerge;
    if (pendingUser && Date.now() - pendingUser.at < OPTIMISTIC_USER_MERGE_TTL_MS) {
      ...
      if (pendingText) {
        if (userMessageWithTextExists(merged, pendingText)) {
          (state as ChatState).pendingOptimisticUserMerge = null;
        } else {
          merged = [...merged, pendingUser.message];
        }
      }
    } else if (pendingUser) {
      (state as ChatState).pendingOptimisticUserMerge = null;
    ...
    // Preserve last appended final message if server hasn't caught up yet
    const pending = (state as ChatState).lastAppendedFinalMessage;
    if (pending && Date.now() - pending.at < LAST_APPENDED_FINAL_TTL_MS) {
      const lastMsg = merged[merged.length - 1];
      const lastText = lastMsg ? extractText(lastMsg) : null;
      const pendingText = extractText(pending.message);
      if (pendingText && lastText !== pendingText) {
        merged = [...merged, pending.message];
      } else {
        (state as ChatState).lastAppendedFinalMessage = null;
      }
    } else if (pending) {
      (state as ChatState).lastAppendedFinalMessage = null;
    }
...
export function handleChatEvent(state: ChatState, payload?: ChatEventPayload) {
  ...
  const isOtherRun = Boolean(state.chatRunId && payload.runId !== state.chatRunId);
  if (isOtherRun) {
    ...
    if (payload.state === "delta" || payload.state === "reasoning") {
      state.chatRunId = payload.runId ?? state.chatRunId;
  ...
  if (payload.state === "delta") {
    ...
    if (
      ...
      // From another run (e.g. cron): replace stream. Same run: only update if longer.
      if (isOtherRun) {
    ...
    if (isGhostFinal(payload)) {
      // Ghost finals (no message body) appear during compaction/hybrid retries; do not clear run state.
      return null;
    }
    const finalMessage = normalizeFinalAssistantMessage(payload.message);
    if (finalMessage && !shouldHideAssistantChatMessage(finalMessage)) {
      state.chatMessages = [...state.chatMessages, finalMessage];
      (state as ChatState).lastAppendedFinalMessage = {
        runId: payload.runId ?? "",
