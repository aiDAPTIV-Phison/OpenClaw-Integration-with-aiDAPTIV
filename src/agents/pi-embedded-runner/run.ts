...
function logEffectiveContextWindowHybridCap(
  ctxInfo: { source: string; tokens: number },
  provider: string,
  modelId: string,
): void {
...
export async function runEmbeddedPiAgent(
  ...
  return enqueueSession(() => {
    ...
    return enqueueGlobal(async () => {
      ...
      let ctxInfo = resolvedRuntimeModel.ctxInfo;
        ...
        while (true) {
          ...
          if (
            ...
            if (!hybridGatewayPayloadCloudEscalated && cloudFb && !alreadyOnCloudEscalationTarget) {
              const reResolved = await resolveModelAsync(cloudFb.provider, cloudFb.model, agentDir, params.config);
              if (reResolved.model) {
                const savedRunModel = { provider, modelId, runtimeModel, ctxInfo, effectiveModel };
                provider = cloudFb.provider;
                modelId = cloudFb.model;
                runtimeModel = reResolved.model;
                const newResolved = resolveEffectiveRuntimeModel({
                  cfg: params.config,
                  provider,
                  contextConfigProvider: resolveHarnessContextConfigProvider({
                    provider,
                ...
                ctxInfo = newResolved.ctxInfo;
                effectiveModel = newResolved.effectiveModel;
                logEffectiveContextWindowHybridCap(ctxInfo, provider, modelId);
                ...
                try {
                  ...
                  runtimeModel = savedRunModel.runtimeModel;
                  ctxInfo = savedRunModel.ctxInfo;
                  effectiveModel = savedRunModel.effectiveModel;
                  log.warn(
                    `[hybrid-gw] cloud escalation (payload guard): credentials unavailable (${formatErrorMessage(keyErr)}); surfacing error`,
                  );
                }
              }
            }
          ...
          if (contextOverflowError) {
            ...
            const cloudFbForOverflow = getHybridGatewayCloudFallback();
            const alreadyOnOverflowCloudTarget =
              cloudFbForOverflow &&
              normalizeProviderId(provider) === normalizeProviderId(cloudFbForOverflow.provider) &&
              modelId === cloudFbForOverflow.model;
            if (!hybridGatewayOverflowCloudEscalated && cloudFbForOverflow && !alreadyOnOverflowCloudTarget) {
              const reResolvedOverflow = await resolveModelAsync(
                cloudFbForOverflow.provider,
                cloudFbForOverflow.model,
                agentDir,
                params.config,
              );
              if (reResolvedOverflow.model) {
                hybridGatewayOverflowCloudEscalated = true;
                provider = cloudFbForOverflow.provider;
                modelId = cloudFbForOverflow.model;
                runtimeModel = reResolvedOverflow.model;
                const newResolved = resolveEffectiveRuntimeModel({
                  cfg: params.config,
                  provider,
                  contextConfigProvider: resolveHarnessContextConfigProvider({
                ...
                try {
                  ...
                  log.warn(
                    `[hybrid-gw] cloud escape hatch: credentials unavailable (${formatErrorMessage(keyErr)}); surfacing overflow error`,
                  );
                }
              }
            }
